//! MKV muxer: writes compressed packets into a Matroska container.
//!
...
/// An MKV muxer that writes to any `Write + Seek` destination.
pub struct MkvMuxer<W> {
    writer: W,
    tracks: Vec<MuxTrack>,
...
impl<W: Write + Seek> MkvMuxer<W> {
    ...
    fn write_header(&mut self) -> Result<(), MuxError> {
        ...
        let ebml_body = [
            ...
            ebml::string_element(elements::EBML_DOC_TYPE, "matroska"),
    ...
    fn build_info(&mut self) -> Result<Vec<u8>, MuxError> {
        ...
        // Compute the absolute offset of the duration data
    ...
    fn build_track_entry(&self, index: usize, track: &MuxTrack) -> Result<Vec<u8>, MuxError> {
        ...
        entry.extend_from_slice(&ebml::uint_element(elements::FLAG_LACING, 0));
...
fn codec_to_mkv_id(codec: &Codec) -> Result<String, MuxError> {
    match codec {
...
impl<W: Write + Seek> Muxer for MkvMuxer<W> {
    fn add_track(&mut self, info: &TrackInfo) -> Result<TrackIndex, MuxError> {
        ...
        let codec_id = codec_to_mkv_id(&info.codec)?;
