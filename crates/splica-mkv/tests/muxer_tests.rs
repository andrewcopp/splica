//! Integration tests for the MKV muxer.
//!
//! Validates that muxed output can be demuxed back correctly (round-trip).
//! Uses the WebM demuxer for reading since MKV and WebM share the same EBML structure.
...
#[test]
fn test_that_mkv_muxer_produces_demuxable_output() {
    // GIVEN
...
#[test]
fn test_that_mkv_muxer_preserves_keyframe_flags() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    {
        let mut muxer = MkvMuxer::new(&mut output);
...
#[test]
fn test_that_mkv_muxer_preserves_packet_data() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    {
        let mut muxer = MkvMuxer::new(&mut output);
...
#[test]
fn test_that_mkv_muxer_handles_two_tracks() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    {
        let mut muxer = MkvMuxer::new(&mut output);
...
#[test]
fn test_that_mkv_muxer_reads_video_dimensions() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    {
        let mut muxer = MkvMuxer::new(&mut output);
...
#[test]
fn test_that_mkv_muxer_rejects_adding_tracks_after_writing() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    let mut muxer = MkvMuxer::new(&mut output);
    muxer.add_track(&make_video_track(0)).unwrap();
...
#[test]
fn test_that_mkv_muxer_rejects_unsupported_codec() {
    // GIVEN
    let mut output = Cursor::new(Vec::new());
    let mut muxer = MkvMuxer::new(&mut output);
